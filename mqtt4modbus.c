...
void mqtt_connect_callback(struct mosquitto *g_mosq, void *obj, int rc)
{
	...
	if (rc == 0) {
		...
		for (i = 0; g_plc_output[i].identifier[0]; i++) {
			topic = malloc(strlen(MQTT_PREFIX) + strlen(g_plc_output[i].identifier) + strlen(MQTT_MY_PLC_SET) + 1);
		...
		for (i = 0; g_plc_output[i].identifier[0]; i++) {
			topic = malloc(strlen(MQTT_PREFIX) + strlen(g_plc_output[i].identifier) + strlen(MQTT_MY_PLC_GET) + 1);
		...
		for (i = 0; g_plc_output[i].identifier[0]; i++) {
			topic = malloc(strlen(MQTT_PREFIX) + strlen(g_plc_output[i].identifier) + strlen(MQTT_MY_PLC_TOG) + 1);
...
void mqtt_message_callback(struct mosquitto *g_mosq, void *obj, const struct mosquitto_message *message)
{
	...
	for (i = 0; g_plc_output[i].identifier[0]; i++) {
		...
		if (strcmp(message->topic, topic) == 0) {
			...
			if (ok) {
				response = malloc(strlen(topic) + strlen(MQTT_MY_PLC_OK) + 1);
